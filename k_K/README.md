...
```bash
# 克隆项目
git clone https://github.com/<your-username>/kk_chat.git
cd kk_chat
...
```bash
# 构建 initramfs（内含 busybox + kk_chat 二进制）
...
└── scripts/
    ├── build_initramfs.sh  # initramfs 构建（busybox + 二进制 + init 脚本）
    └── build_images.sh     # UEFI/ISO 镜像构建（grub-mkstandalone + xorriso）
```
